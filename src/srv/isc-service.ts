...
export class IscService {
    ...
    async listSourcesByNames(sourceNames: string[]): Promise<Source[]> {
    ...
    async getSourceIdsByNames(sourceNames: string[]): Promise<string[]> {
    ...
    async searchIdentitiesByQuery(identitySearchQuery: string, includedAttributes?: string[]): Promise<IdentityDocument[]> {
    ...
    async searchIdentitiesByIds(identityIds: string[], includedAttributes?: string[]): Promise<IdentityDocument[]> {
    ...
    // Return array of Identity IDs from Search Query
    async getIdentityIdsBySearchQuery(identitySearchQuery: string): Promise<string[]> {
        if (!identitySearchQuery) return []
    ...
    async listAccountsByFilter(accountsFilter: string): Promise<Account[]> {
    ...
    async listAccountsByNativeIdentities(nativeIdentities: string[], correlated?: boolean, chunkSize?: number): Promise<Account[]> {
    ...
    async listAccountsByIdentityAndSourceIds(identityId: string, sourceIds: string[]): Promise<Account[]> {
    ...
    async listAccountsByIdentityIdAndSourceNames(identityId: string, sourceNames: string[]): Promise<Account[]> {
    ...
    async listEntitlementsByAccountId(accountId: string): Promise<EntitlementDto[]> {
        const accountsApi = new AccountsApi(this.getApiConfig())
        try {
    ...
    async searchEntitlementsByQuery(entitlementsQuery: string): Promise<EntitlementDocument[]> {
        const searchApi = new SearchApi(this.getApiConfig())
        const search: Search = {
            indices: [
                Index.Entitlements
    ...
    filterSearchResultsbyCloudEnabled(entitlements: EntitlementDocument[]): EntitlementDocument[] {
    ...
    // Filter incoming list of cloud entitlements by cloudGoverned
    filterAccountEntitlementsbyCloudEnabled(entitlements: EntitlementDto[]): EntitlementDto[] {
        if (entitlements && entitlements.length > 0) {
            const cloudEnabledEntitlements = entitlements.filter(entitlement => entitlement.cloudGoverned)
            if (cloudEnabledEntitlements && cloudEnabledEntitlements.length > 0) {
                this.logger.debug(`Found a total of [${cloudEnabledEntitlements.length}] Cloud Enabled Entitlements`)
    ...
    async searchCloudEnabledEntitlements(sourceNames: string[]): Promise<EntitlementDocument[]> {
        const sourceIds = await this.getSourceIdsByNames(sourceNames)
        if (!sourceIds || sourceIds.length == 0) return []
    ...
    async listCGEntitlementsForAccount(accountId: string): Promise<EntitlementDto[]> {
        if (!accountId) return []
    ...
    async listCGEntitlementsForIdentity(identityId: string, sourceNames: string[]): Promise<EntitlementDto[]> {
    ...
    async listCGEntitlementsForIdentitiesBySourceIds(identityIds: string[], sourceIds: string[]): Promise<EntitlementDto[]> {
    ...
    async listCGEntitlementsForIdentitiesBySourceNames(identityIds: string[], sourceNames: string[]): Promise<EntitlementDto[]> {
        const sourceIds = await this.getSourceIdsByNames(sourceNames)
        if (!sourceIds || sourceIds.length == 0) return []
    ...
    async listCGEntitlementsForSearchIdentitiesBySourceNames(identitySearchQuery: string, sourceNames: string[]): Promise<EntitlementDto[]> {
    ...
    async searchAccessProfilesbyEntitlements(apiConfig: Configuration, entitlements: any[]): Promise<AccessProfileDocument[]> {
        ...
        const query = arrayFunc.buildQueryOrFilter(arrayFunc.buildIdArray(entitlements, true), 'id:', ' OR ', false, '@entitlements(', ')')
        ...
        try {
            ...
            logSrv.logStandardApiError(this.logger, error, 'Search', 'finding access profiles from entitlements', search)
            return []
        }
    }
    ...
    async searchRolesByAccessProfilesOrEntitlements(apiConfig: Configuration, entitlements: any[], accessProfiles: any[]): Promise<RoleDocument[]> {
        let query
        if (entitlements && entitlements.length > 0) {
            query = arrayFunc.buildQueryOrFilter(arrayFunc.buildIdArray(entitlements, true), 'id:', ' OR ', false, '@entitlements(', ')')
        }
        if (accessProfiles && accessProfiles.length > 0) {
            if (!query) {
                query = arrayFunc.buildQueryOrFilter(arrayFunc.buildIdArray(accessProfiles, true), 'accessProfiles.id:', ' OR ', false)
            } else {
                query += arrayFunc.buildQueryOrFilter(arrayFunc.buildIdArray(accessProfiles, true), 'accessProfiles.id:', ' OR ', false, ' OR ')
            }
        }
        if (!query) return []
        const searchApi = new SearchApi(apiConfig)
        ...
        try {
            ...
            logSrv.logStandardApiError(this.logger, error, 'Search', 'finding roles from access profiles and entitlements', search)
            return []
        }
    }
